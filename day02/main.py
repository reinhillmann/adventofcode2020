...
def correct_passwords_part_1(input):
    ...
    for line in input:
        (mincount, maxcount, letter, password) = parse_password(line)
        if password is None:
            continue
        count = 0
...
def correct_passwords_part_2(input):
    correct_password_count = 0
    for line in input:
        (pos1, pos2, letter, password) = parse_password(line)
        if password is None:
            continue
        pos1 -= 1
        pos2 -= 1
        is_correct = ((password[pos1] == letter or password[pos2] == letter) and not
...
4-11 p: jzpprcpcmmpb
9-10 f: fffpffffjff
1-5 d: ndtdc""".splitlines()

    assert correct_passwords_part_1(test_input) == 2
    assert correct_passwords_part_2(test_input) == 1

    print(correct_passwords_part_1(actual_input))
    print(correct_passwords_part_2(actual_input))
